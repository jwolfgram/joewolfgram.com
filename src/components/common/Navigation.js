...
export default class Navigation extends Component {
    ...
    componentDidMount() {
        // if (browserHistory.location.pathname !== '/welcome') {
    ...
    showNavigationLink() { //This also seems to be handling the actual implementation of which navbar to display (Menu items or show login only on navbar)
        ...
        if (browserHistory.location.pathname == '/welcome') { //IF no active menu items then setState to show login only
            return (
                <div id="myNavbar">
                    <ul className="nav navbar-nav navbar-right">
                        <li>
                            <a role="button" onClick={() => {this.setState({showUserDropdown: !this.state.showUserDropdown})}}>
        ...
        } else {   //setState to show login only to FALSE
            return (
                <Navbar.Collapse>
                        ...
                        {this.state.navigationLinks.map((menuItems, i) => {
                            if (menuItems.hasOwnProperty('isActive') && menuItems['navbarItem']) {
                                if (menuItems.isActive === true) {
                                    return (<li role="presentation" className="active" key={i + "_navItem"} href='#'>
                                                <Link key={i} to={menuItems.href}>{menuItems.name}</Link>
                                            </li>);
                                } else {
                                    return (<li role="presentation" key={i + "_navItem"} href='#'>
                                                <Link key={i} to={menuItems.href}>{menuItems.name}</Link>
                                            </li>);
                                }
                            }
